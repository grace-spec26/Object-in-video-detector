...
3. You can [install](#installation-instructions) CoTracker _locally_ and then:
   - Run an _offline_ demo with 10 ⨉ 10 points sampled on a grid on the first frame of a video (results will be saved to `./saved_videos/demo.mp4`)):

     ```bash
     python demo.py --grid_size 10
     ```

   - Run an _online_ demo:
...
cd co-tracker
pip install - matplotlib flow_vis tqdm matplotlib flow_vis tqdm tensorboard gradio mediapy opencv-python torch tim
```
